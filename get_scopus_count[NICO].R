...
# create the exponential equation text
intercept_value <- exp(coefficients[1])
intercept_coef <- format(intercept_value, scientific = TRUE, digits = 3)
intercept_parts <- strsplit(intercept_coef, "e")[[1]]
intercept_base <- gsub("\\.", ",", intercept_parts[1])
intercept_exponent <- intercept_parts[2]
slope <- gsub("\\.", ",", round(coefficients[2], 2))

# construct equation
exp_eq <- bquote(italic(y) == .(intercept_base) %.% 10^.(intercept_exponent) * italic(e)^{.(slope) * italic(x)})
